-- Migration: cria form "Partners LP" no schema compartilhado de laquila-forms.
-- Motivo: partners.laquilamarketing.com.br/api/leads passa a fazer dual-write —
--   continua gravando em public.leads (consumido por email-whatsapp-mkt/laquila-growth)
--   E grava em form_responses (pra aparecer no dashboard forms.laquilacompany.app).
-- Idempotente: skip se slug já existir.
...
declare
  ...
  -- UUIDs dos 14 blocks (determinísticos via gen_random_uuid dentro do DO)
...
begin
  ...
  if v_form is not null then
    raise notice 'Partners LP form já existe: id=%', v_form;
    return;
  end if;

  -- Workspace + created_by do primeiro form existente
  select workspace_id, created_by into v_workspace, v_user
    from forms order by created_at asc limit 1;

  if v_workspace is null then
    raise exception 'Nenhum form existente pra herdar workspace_id/created_by. Aborting.';
  end if;
  ...
  values (
    ...
    jsonb_build_object(
      ...
      'thankYouScreen', jsonb_build_object(
        ...
        'message', 'Nosso time analisa e entra em contato em até 48 horas úteis. Você não precisa marcar horário.'
      ),
  ...
  -- INSERT blocks (ordem corresponde à do form em /site/app/components/ApplicationForm.tsx)
  insert into form_blocks (id, form_id, type, title, description, required, order_index, config) values
    ...
    (v_escritorio, v_form, 'short_text', 'Nome do escritório', null, true, 4,
     jsonb_build_object('placeholder', 'Escritório Sobrenome Advocacia')),
    ...
    (v_funcionarios, v_form, 'multiple_choice', 'Tamanho do escritório', null, true, 7,
     jsonb_build_object(
       'allowMultiple', false,
       'choices', jsonb_build_array(
         jsonb_build_object('id','func-solo','label','Atuo sozinho(a)','value','solo'),
         jsonb_build_object('id','func-2-3','label','2 a 3 advogados','value','2-3'),
         jsonb_build_object('id','func-4p','label','4 ou mais advogados','value','4+')
       ))),
    ...
    (v_tempo, v_form, 'multiple_choice', 'Há quanto tempo você investe em marketing digital?', null, true, 8,
     jsonb_build_object(
       'allowMultiple', false,
       'choices', jsonb_build_array(
         jsonb_build_object('id','tempo-0','label','Menos de 6 meses','value','ate6m'),
         jsonb_build_object('id','tempo-1','label','6 a 12 meses','value','6a12m'),
         jsonb_build_object('id','tempo-2','label','1 a 2 anos','value','1a2a'),
         jsonb_build_object('id','tempo-3','label','Mais de 2 anos','value','+2a')
       ))),
    ...
    (v_investimento, v_form, 'multiple_choice', 'Investimento mensal em tráfego pago', null, true, 10,
     jsonb_build_object(
       'allowMultiple', false,
       'choices', jsonb_build_array(
         jsonb_build_object('id','inv-0','label','Até R$ 5 mil','value','<5k'),
         jsonb_build_object('id','inv-1','label','R$ 5 mil a R$ 15 mil','value','5a15k'),
         jsonb_build_object('id','inv-2','label','R$ 15 mil a R$ 30 mil','value','15a30k'),
         jsonb_build_object('id','inv-3','label','R$ 30 mil a R$ 60 mil','value','30a60k'),
         jsonb_build_object('id','inv-4','label','Mais de R$ 60 mil','value','+60k')
       ))),
    ...
    (v_quem_roda, v_form, 'multiple_choice', 'Quem roda seu marketing hoje?', null, true, 11,
     jsonb_build_object(
       'allowMultiple', false,
       'choices', jsonb_build_array(
         jsonb_build_object('id','qr-0','label','Agência','value','agencia'),
         jsonb_build_object('id','qr-1','label','Freelancer / consultor','value','freela'),
         jsonb_build_object('id','qr-2','label','Equipe interna (in-house)','value','inhouse'),
         jsonb_build_object('id','qr-3','label','Eu mesmo','value','eu'),
         jsonb_build_object('id','qr-4','label','Ninguém / sem estrutura','value','nenhum')
       ))),
    ...
    (v_aceita, v_form, 'multiple_choice', 'Você entende e aceita o modelo de comissão sobre contratos fechados?', null, true, 14,
     jsonb_build_object(
       'allowMultiple', false,
       'choices', jsonb_build_array(
         jsonb_build_object('id','com-0','label','Sim, é exatamente isso que estou buscando','value','sim'),
         jsonb_build_object('id','com-1','label','Quero entender melhor na conversa','value','entender'),
         jsonb_build_object('id','com-2','label','Prefiro explorar a modalidade híbrida (mensalidade + comissão)','value','hibrido'),
         jsonb_build_object('id','com-3','label','Não tenho interesse nesse formato','value','nao')
       )));

  -- Atualiza settings.lead_mapping com os UUIDs dos blocks principais
  -- (usado pelo syncToDailyLeads do laquila-forms pra extrair campos canônicos)
  update forms
     set settings = settings || jsonb_build_object(
       ...
       'partners_block_map', jsonb_build_object(
         ...
         'email',                 v_email::text,
         'phone',                 v_phone::text,
         'cidade_uf',             v_cidade::text,
         'escritorio',            v_escritorio::text,
         'instagram',             v_instagram::text,
         'area',                  v_area::text,
         'funcionarios_escritorio', v_funcionarios::text,
         'tempo_investimento',    v_tempo::text,
         'contratos_mes',         v_contratos::text,
         'investimento_trafego',  v_investimento::text,
         'quem_roda_marketing',   v_quem_roda::text,
         'motivo',                v_motivo::text,
         'ambicao_12m',           v_ambicao::text,
         'aceita_comissao',       v_aceita::text
       )
     )
   where id = v_form;

  raise notice 'Partners LP form criado: id=%, slug=partners-lp, 15 blocks (14 required + 1 opcional)', v_form;
end $$;
